...

{
	...
	public TeleporterTrit(WorldServer worldserver) {
		super(worldserver);
			...
			for(int l = -2; l <= 2; ++l)
			{
				for(int i1 = -2; i1 <= 2; ++i1)
				{
					for(int j1 = -1; j < 3; ++j1)
					{
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.WorldServerInstance.setBlock(k1, l1, i2, flag ? ModBlocks.tritOre : Blocks.air);
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8)
	{
		...
		{
			TeleporterTrit.PortalPosition portalposition = (Teleporter.PortalPosition)this.destinationCoordinateCache.getValueByKey(j1);
			d3 = 0.0;
